...
class TurnoverPredictor:
    ...
    def __init__(
        ...
        print(f"TurnoverPredictor initialisé")
        print(f"  Modèle: {self.model_path.name}")
        print(f"  Seuil: {self.threshold:.4f}")
    ...
    def _load_threshold(self) -> float:
        ...
        if not self.threshold_path.exists():
            print(f"Warning: Seuil non trouvé: {self.threshold_path}, utilisation de 0.5 par défaut")
            return 0.5
    ...
    def _load_schema(self) -> Dict:
        ...
        if not self.schema_path.exists():
            print(f"Warning: Schéma non trouvé: {self.schema_path}")
            return {}
    ...
    def predict_batch(self, data: Union[pd.DataFrame, str]) -> pd.DataFrame:
        ...
        if isinstance(data, str):
            ...
            print(f"{len(data)} observations chargées depuis {data}")
