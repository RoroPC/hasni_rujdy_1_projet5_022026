...
class TurnoverDataPreprocessor:
    ...
    def __init__(self):
        ...
        self.colonnes_exclues = [
            ...
            'a_quitte_l_entreprise', 'a_quitte_num',
    ...
    def handle_missing_values(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if removed_rows > 0:
            print(f"Warning: {removed_rows} lignes supprimées (valeurs manquantes)")
        ...
        for col in self.variables_binaires:
            if col in df.columns:
                ...
                if df[col].isnull().any():
                    print(f"Warning: Valeurs non reconnues dans {col}")
    ...
    def encode_categorical_variables(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Identifier les variables catégorielles existantes
        variables_onehot_existantes = [
            col for col in self.variables_onehot if col in df.columns
        ]
        
        if variables_onehot_existantes:
            df = pd.get_dummies(
                df,
                columns=variables_onehot_existantes,
                drop_first=True,
                prefix_sep='_'
            )
    ...
    def create_engineered_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'revenu_mensuel' in df.columns and 'age' in df.columns:
            ...
            df['salaire_par_age'] = df.apply(
                lambda row: row['revenu_mensuel'] / row['age'] if row['age'] > 0 else 0,
                axis=1
            )
        ...
        if 'annees_dans_le_poste_actuel' in df.columns and 'annee_experience_totale' in df.columns:
            df['stagnation_poste'] = df.apply(
                lambda row: row['annees_dans_le_poste_actuel'] / row['annee_experience_totale'] 
                if row['annee_experience_totale'] > 0 else 0,
                axis=1
            )
    ...
    def validate_data(self, df: pd.DataFrame) -> Tuple[bool, List[str]]:
        ...
        binary_features = [
            'heure_supplementaires', 'promotion_recente', 'distance_longue',
    ...
    def transform(self, df: pd.DataFrame, validate: bool = True) -> pd.DataFrame:
        ...
        if validate:
            ...
            if not is_valid:
                ...
                for error in errors:
                    print(f"  - {error}")
                raise ValueError("Données invalides après preprocessing")
        
        print(f"Preprocessing terminé: {df_processed.shape[0]} observations, {df_processed.shape[1]} features")
...
def load_and_prepare_csv(filepath: str, has_target: bool = False) -> Tuple[pd.DataFrame, Optional[pd.Series]]:
    """
    ...
    # Preprocessing
    preprocessor = TurnoverDataPreprocessor()
    X = preprocessor.transform(df, validate=True)
